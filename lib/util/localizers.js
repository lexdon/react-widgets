...
var DateLocalizer = function DateLocalizer(spec) {
  ...
  this.format = function (value, format, culture, presenter) {
    return _format(_this2, spec.format, value, format, culture, presenter);
  };

  this.parse = function (value, format, culture, dateValidator) {
    var result = spec.parse.call(_this2, value, format, culture, dateValidator);

    invariant(result == null || typeof result === 'string' || result instanceof Date && !isNaN(result.getTime()), 'date localizer `parse(..)` must return a valid Date, string, null, or undefined');
